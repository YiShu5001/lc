...
class BaseAviary(gym.Env):
    ...
    def __init__(self,
        ...
        #  删除了 部分 在 TODO vision 中的内容，因为我不需要 vision ，所以就没有必要了
        #### Connect to PyBullet   连接到仿真gui（图形系统）或者数字系统###################################
        '''
        该部分内容 全部都是解释两种仿真 （ 图像 数据 ）情况下  解读图像数据的过程 对我无用 参数 ： ret；RECORD
        '''
        if self.GUI:
            #### With debug GUI ####################################### 其实我觉得没必要，可能图像分析需要通过这个增强数据信息#
            self.CLIENT = p.connect(p.GUI) # p.connect(p.GUI, options="--opengl2")

            ###  设置相机视角 （ 距离 、 角度 、 位置 ）  ，  我设置的是  从 上方 看  ，  距离 8 ，  角度 -30 ， 位置 [0,0,1]
    ...
    def step(self,
        ...
        for _ in range(self.PYB_STEPS_PER_CTRL):  # 每次控制频率  仿真频率/控制频率 ， 一次动作保持多少步长
            ...
            # TODO： 不同仿真环境调用不同的形式， 是叠加状态: 基础物理模型之后  加上 地面效应、空气阻力 等等
            # ##
            for i in range (self.NUM_DRONES):
    ...
    def _getAdjacencyMatrix(self):
        ...
        """
        adjacency_mat = np.identity(self.NUM_DRONES)    #对角阵
