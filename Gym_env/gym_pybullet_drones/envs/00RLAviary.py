...
from gym_pybullet_drones.envs.BaseAviarySimple import BaseAviary
...
class BaseRLAviary(BaseAviary):
    ...
    def _actionSpace(self):
        ...
        """
        if self.ACT_TYPE in [ActionType.RPM, ActionType.VEL]:    # 4个电机的RPM，或者三维速度（方向）+速度系数（快慢）
            size = 4
        elif self.ACT_TYPE==ActionType.PID:                  # 三维位置控制（输入为目标位置）
            size = 3
        elif self.ACT_TYPE in [ActionType.ONE_D_RPM, ActionType.ONE_D_PID]: # 统一转速
            size = 1
        else:
    ...
    def _preprocessAction(self,action):
        ...
        for k in range(action.shape[0]):   #第几个无人机
            target = action[k, :]
    ...
    def _observationSpace(self):
        ...
        if self.OBS_TYPE == ObservationType.RGB:
            ...
            obs_lower_bound = np.hstack(
                [obs_lower_bound, np.array([[lo,lo,0, lo,lo,lo,lo,lo,lo,lo,lo,lo] for i in range(self.Obs)])])
            obs_upper_bound = np.hstack(
                [obs_upper_bound, np.array([[lo,lo,0, lo,lo,lo,lo,lo,lo,lo,lo,lo] for i in range(self.Obs)])])
    ...
    def _computeObs(self):
        ...
        if self.OBS_TYPE == ObservationType.RGB:
            ...
            #### 添加障碍物到观测空间  ###
            obs_12 = np.zeros((self.Obs,12))
            for i in range(self.Obs):
                obs = self._getObstacleStateVector(i)
                obs_12[i, :] = np.hstack([obs[0:3], obs[7:10], obs[10:13], obs[13:16]]).reshape(12,)
            ret = np.array([obs_12[i, :] for i in range(self.NUM_DRONES)]).astype('float32')
