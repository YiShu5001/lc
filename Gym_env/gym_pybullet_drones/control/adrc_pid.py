...
class LADRC:
    ...
    def LESO(self, current):
        ...
        self.err = current - self.x1
        self.x1 += (self.x2 + self.l1 * self.err) * self.h
        self.x2 += (self.x3 + self.l2 * self.err + self.b0 * self.u) * self.h
        self.x3 += self.l3 * self.err * self.h
    ...
    def PD(self):
        ...
        self.e1 = self.v1 - self.x1
        self.e2 = self.v2 - self.x2
        self.u0 = self.kp * self.e1 + self.kd * self.e2
        # 也补充上了最后一点的u0转变为u的过程
    ...
    def _log_parameters(self):
        """记录当前所有参数值到logger"""
        self.logger['wc'].append(self.wc)
        self.logger['b0'].append(self.b0)
        self.logger['w0'].append(self.w0)
        self.logger['h'].append(self.h)
        self.logger['r'].append(self.r)
        self.logger['v1'].append(self.v1)
        self.logger['v2'].append(self.v2)
        self.logger['l1'].append(self.l1)
        self.logger['l2'].append(self.l2)
        self.logger['l3'].append(self.l3)
        self.logger['x1'].append(self.x1)
        self.logger['x2'].append(self.x2)
        self.logger['x3'].append(self.x3)
        self.logger['kp'].append(self.kp)
        self.logger['kd'].append(self.kd)
        self.logger['u0'].append(self.u0)
        self.logger['u'].append(self.u)
        self.logger['current'].append(self.current)
        self.logger['err'].append(self.err)
        self.logger['e1'].append(self.e1)
        self.logger['e2'].append(self.e2)
        self.logger['kp_u'].append(self.kp * self.e1)
        self.logger['kd_u'].append(self.kd * self.e2)
...
class LADRCControl(BaseControl):
    ...
    def __init__(self,
        ...
        if self.DRONE_MODEL != DroneModel.CF2X and self.DRONE_MODEL != DroneModel.CF2P:
            ...
            exit()
        # 1. 默认参数：6个通道 [X, Y, Z, 横滚, 俯仰, 偏航]，每个通道为 [wc, b0, w0]2
        wc = 5.20
        LADRC_para = [
            np.array([1.1, 220., 9.6, 0.005, 30.0]),  # X通道默认参数
            np.array([1.3, 220., 9.6, 0.005, 30.0]),  # Y通道默认参数
            np.array([15., 300.0, 110., 0.005, 30.0]),  # Z通道默认参数
            np.array([6., 250.0, 30, 0.005, 50.0]),  # 横滚通道默认参数
            np.array([6., 250.0, 30, 0.005, 50.0]),  # 俯仰通道默认参数
            np.array([1.2, 150.0, 6.0, 0.005, 50.0])  # 偏航通道默认参数（较低响应避免震荡）
        ]
        ...
        self.PID_para = [
            np.array([0.4, .05, .2]),  # X通道默认参数
            np.array([0.4, .05, .2]),  # Y通道默认参数
            np.array([1.25, .05, .5]),  # Z通道默认参数
            np.array([70000.0, .0, 20000.0]),  # 横滚通道默认参数
            np.array([70000.0, .0, 20000.0]),  # 俯仰通道默认参数
            np.array([6000.0, 500.0, 12000.0]),  # 偏航通道默认参数（）
        ]
    ...
    def _dslPIDPositionControl(self,
        ...
        # 拆分为x,y,z轴，三轴分割
        u_x = np.dot(self.PID_para[0], [pos_e[0], self.integral_pos_e[0], vel_e[0]])
        u_y = np.dot(self.PID_para[1], [pos_e[1], self.integral_pos_e[1], vel_e[1]])
        u_z = np.dot(self.PID_para[2], [pos_e[2], self.integral_pos_e[2], vel_e[2]])
        # u_x = self.con_X(target_pos[0], cur_pos[0])
        # u_y = self.con_Y(target_pos[1], cur_pos[1])
        # u_z = self.con_Z(target_pos[2], cur_pos[2])
        ## 3. 目标推力向量计算  （TODO： 姿态解算）
        target_thrust = np.array([u_x, u_y, u_z])+ np.array([0, 0, self.GRAVITY])
    ...
    def _dslPIDAttitudeControl(self,
        ...
        u_roll = np.dot(self.PID_para[3], [-rot_e[0], self.integral_rpy_e[0], rpy_rates_e[0]])
        u_pitch = np.dot(self.PID_para[4], [-rot_e[1], self.integral_rpy_e[1], rpy_rates_e[1]])
        u_yall = np.dot(self.PID_para[5], [-rot_e[2], self.integral_rpy_e[2], rpy_rates_e[2]])

        ### LADRC target torques ####################################
        u_roll = self.con_roll(target_euler[0],cur_rpy[0])
        u_pitch = self.con_pitch(target_euler[1], cur_rpy[1])
        u_yall = self.con_yaw(target_euler[2], cur_rpy[2])
        target_torques = np.array([u_roll, u_pitch, u_yall])*10000
...
if __name__ == '__main__':
    ladrc = LADRC(1.2, 1., 6.0, 0.005, 30)
    # 二阶被控对象参数
    zeta = 0.2         # 阻尼比
    omega_n = 0.8      # 固有频率
    K = 2.2            # 增益
    ...
    axes[2].plot(t, out_hat, 'b--', label='TD微分输出 v1')
