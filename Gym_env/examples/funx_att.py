...
from gym_pybullet_drones.control.LADRC import LADRCControl, LADRC
...
def run(
    ...
    NUM_WP = control_freq_hz*PERIOD
    TARGET_POS = np.zeros((NUM_WP, 2))
    CENTER = np.array([1, 0])  # 圆心坐标
    RADIUS = 0.35  # 圆的半径
    for i in range(NUM_WP):
        angle = 2 * np.pi * (i / NUM_WP)  # 计算当前角度
        # 根据圆的参数方程计算目标位置
        #TARGET_POS[i, :] = CENTER + RADIUS * np.array([np.cos(angle), np.sin(angle)])
    wp_counters = np.array([0, int(NUM_WP/2)])
    ...
    ctrl = [LADRCControl(drone_model=drone) for i in range(1)]
    ...
    for i in range(0, int(duration_sec*env.CTRL_FREQ)):
        ...
        for j in range(2):
            action[j, :], _, _ = ctrl[j].computeControlFromState(control_timestep=env.CTRL_TIMESTEP,
                                                                    state=obs[j],
                                                                    target_pos=np.hstack([TARGET_POS[wp_counters[j], :], INIT_XYZS[j, 1]]),
                                                                    )
