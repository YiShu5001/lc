...
WITH_R_LABEL = "ladrc_retuned_r63_no_td"
NO_R_LABEL = "ladrc_retuned_no_r_search_no_td"
...
def _controller_display_name(label: str) -> str:
    return {
        PID_LABEL: "PID",
        WITH_R_LABEL: "LADRC(r=63, no TD)",
        NO_R_LABEL: "LADRC(no-r, no TD)",
    }[label]
...
def _apply_fixed_ladrc(candidate: FixedLADRCCandidate):
    ...
    _disable_td_for_axis(controller, "x")
    return controller
...
def main() -> None:
    ...
    output_root = ensure_dir(Path("outputs") / "control_pybullet" / "x_speed_sweep_pid_vs_ladrc_no_td_compare" / axis / stamp)
    ...
    summary = {
        "axis": axis,
        "scenario_name": "x_speed_sweep_pid_vs_ladrc_no_td_compare",
        "backend": "gym_env",
