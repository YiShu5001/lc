...
def _plot_figure_3_2(input_root: Path, out_dir: Path) -> None:
    ...
    axes[0].plot(step_df.iloc[:, 0], step_df.iloc[:, 1], "--", color=COLORS["Reference"], linewidth=2.0, label="Reference")
    ...
    axes[0].set_title("(a) Step response comparison")
    axes[0].set_xlabel("Time / s")
    axes[0].set_ylabel("Position / m")
    axes[0].grid(alpha=0.25)
    axes[0].legend(fontsize=9)

    axes[1].plot(disturb_df.iloc[:, 0], disturb_df.iloc[:, 1], "--", color=COLORS["Reference"], linewidth=2.0, label="Reference")
    ...
    if nonzero.any():
        ...
        axes[1].axvline(inj_t.iloc[0], color="#d62728", linestyle="--", linewidth=1.6, label="Disturbance on")
        axes[1].axvline(inj_t.iloc[-1], color="#d62728", linestyle=":", linewidth=1.4, label="Disturbance off")
        axes[1].axvspan(inj_t.iloc[0], inj_t.iloc[-1], color="#d62728", alpha=0.08)
    axes[1].set_title("(b) Disturbance recovery comparison")
    ...
    axes[2].set_title("(c) Constant-speed tracking error")
    axes[2].set_xlabel("Time / s")
    axes[2].set_ylabel("Error / m")
    axes[2].grid(alpha=0.25)
    axes[2].legend(fontsize=9)

    fig.suptitle("Fig. 3-2 Performance comparison of fixed-parameter LADRC under multiple operating conditions", fontsize=14, y=1.03)
    _save(fig, out_dir / "Fig_3-2_Performance_comparison_of_fixed_parameter_LADRC_under_multiple_operating_conditions")
...
def _plot_figure_3_5(input_root: Path, out_dir: Path) -> None:
    ...
    stage_names = ["Forward", "Hover", "Reverse", "Final hold"]
    ...
    if not dist_start.empty and not dist_end.empty:
        ...
        ax.text((dist_start.iloc[0] + dist_end.iloc[0]) / 2, y_dist, "Random force disturbance", ha="center", va="center", fontsize=8.5, color="white")
    ...
    ax.set_yticklabels(["Reference profile", "Disturbance injection", "Recovery interval"])
    ax.set_xlabel("Time / s")
    ax.set_title("Fig. 3-5 Timeline of control-layer operating conditions and disturbance injection")
    ax.set_xlim(0.0, max(stage_ends))
    ax.grid(axis="x", alpha=0.25)
    _save(fig, out_dir / "Fig_3-5_Timeline_of_control_layer_operating_conditions_and_disturbance_injection")
...
def _plot_figure_3_6(input_root: Path, out_dir: Path) -> None:
    ...
    label_map = {speed_df.columns[2]: "PID", speed_df.columns[3]: "Fixed LADRC", speed_df.columns[4]: "DDPG--RL--LADRC"}
    label_map_b = {recovery_df.columns[3]: "PID", recovery_df.columns[4]: "Fixed LADRC", recovery_df.columns[5]: "DDPG--RL--LADRC"}

    fig, axes = plt.subplots(2, 2, figsize=(14, 9))

    axes[0, 0].plot(speed_df.iloc[:, 0], speed_df.iloc[:, 1], "--", color=COLORS["Reference"], linewidth=2.0, label="Reference velocity")
    for col in speed_df.columns[2:]:
        label = label_map[col]
        axes[0, 0].plot(speed_df.iloc[:, 0], speed_df[col], linewidth=2.0, label=label, color=COLORS[label])
    axes[0, 0].set_title("(a) Reference velocity tracking")
    axes[0, 0].set_xlabel("Time / s")
    axes[0, 0].set_ylabel("Velocity / m/s")
    axes[0, 0].grid(alpha=0.25)
    axes[0, 0].legend(fontsize=9)

    axes[0, 1].plot(recovery_df.iloc[:, 0], recovery_df.iloc[:, 1], "--", color=COLORS["Reference"], linewidth=2.0, label="Reference position")
    ...
    if nonzero.any():
        ...
        axes[0, 1].axvline(inj_t.iloc[0], color="#d62728", linestyle="--", linewidth=1.5, label="Disturbance on")
        axes[0, 1].axvline(inj_t.iloc[-1], color="#d62728", linestyle=":", linewidth=1.4, label="Disturbance off")
        axes[0, 1].axvspan(inj_t.iloc[0], inj_t.iloc[-1], color="#d62728", alpha=0.08)
    axes[0, 1].set_title("(b) Disturbance recovery response")
    ...
    width = 0.36
    ax_c = axes[1, 0]
    bars1 = ax_c.bar(x - width / 2, smooth_df["Control input variance"], width=width, color="#4c78a8")
    ax_c2 = ax_c.twinx()
    bars2 = ax_c2.bar(x + width / 2, smooth_df["Mean |Δu|"], width=width, color="#f58518")
    ax_c.set_xticks(x)
    ax_c.set_xticklabels(smooth_df["Method"], rotation=0)
    ax_c.set_title("(c) Control-input smoothness")
    ax_c.set_ylabel("Control input variance")
    ax_c2.set_ylabel("Mean |Δu|")
    ax_c.grid(axis="y", alpha=0.2)
    ax_c.legend([bars1, bars2], ["Control input variance", "Mean |Δu|"], loc="upper left", fontsize=9)

    axes[1, 1].plot(reward_df["episode"], reward_df["reward"], color="#bdbdbd", linewidth=1.0, alpha=0.6, label="Episode reward")
    axes[1, 1].plot(reward_df["episode"], reward_df["average_reward"], color="#2ca02c", linewidth=2.0, label="Average reward")
    axes[1, 1].set_title("(d) DDPG training reward convergence")
    axes[1, 1].set_xlabel("Episode")
    axes[1, 1].set_ylabel("Reward")
    axes[1, 1].grid(alpha=0.25)
    axes[1, 1].legend(fontsize=9)

    fig.suptitle("Fig. 3-6 Comprehensive performance comparison of DDPG--RL--LADRC", fontsize=14, y=1.02)
    _save(fig, out_dir / "Fig_3-6_Comprehensive_performance_comparison_of_DDPG--RL--LADRC")
...
def _plot_figure_3_7(input_root: Path, out_dir: Path, ablation_root: Path) -> None:
    ...
    reward_df.columns = ["Method", "episode", "reward", "average_reward"]
    metrics_df.columns = ["Method", "RMSE", "ITAE", "Maximum deviation", "Recovery time", "Control input variance", "Mean |Δu|", "MAE", "velocity_rmse"]
    smooth_df.columns = ["Method", "Control input variance", "Mean |Δu|"]

    replace_method = {
        "完整方法": "Full method",
        "瀹屾暣鏂规硶": "Full method",
        "w/o鐘舵€佸彔鍔?": "w/o state stacking",
        "w/o状态叠加": "w/o state stacking",
        "w/o鍔ㄤ綔淇濇寔": "w/o action hold",
        "w/o动作保持": "w/o action hold",
        "w/o N-step": "w/o N-step",
    }
    for df in (reward_df, metrics_df, smooth_df):
        df["Method"] = df["Method"].replace(replace_method)

    method_order = ["Full method", "w/o state stacking", "w/o action hold", "w/o N-step"]
    fig, axes = plt.subplots(1, 3, figsize=(18, 4.8))

    for method in method_order:
        part = reward_df[reward_df["Method"] == method]
        axes[0].plot(part["episode"], part["average_reward"], linewidth=2.0, label=method, color=COLORS[method])
    axes[0].set_title("(a) Training reward curves")
    axes[0].set_xlabel("Episode")
    axes[0].set_ylabel("Average reward")
    axes[0].grid(alpha=0.25)
    axes[0].legend(fontsize=9)

    metric_plot = metrics_df.set_index("Method").loc[method_order].reset_index()
    x = np.arange(len(metric_plot))
    axes[1].bar(x, metric_plot["RMSE"], color=[COLORS[m] for m in metric_plot["Method"]], width=0.6)
    axes[1].set_xticks(x)
    axes[1].set_xticklabels(metric_plot["Method"], rotation=15)
    axes[1].set_title("(b) Recovery RMSE comparison")
    axes[1].set_ylabel("RMSE")
    axes[1].grid(axis="y", alpha=0.25)

    smooth_plot = smooth_df.set_index("Method").loc[method_order].reset_index()
    x2 = np.arange(len(smooth_plot))
    width = 0.35
    axes[2].bar(x2 - width / 2, smooth_plot["Control input variance"], width=width, color="#4c78a8", label="Control input variance")
    ax2b = axes[2].twinx()
    ax2b.bar(x2 + width / 2, smooth_plot["Mean |Δu|"], width=width, color="#f58518", label="Mean |Δu|")
    axes[2].set_xticks(x2)
    axes[2].set_xticklabels(smooth_plot["Method"], rotation=15)
    axes[2].set_title("(c) Control-input smoothness")
    axes[2].set_ylabel("Control input variance")
    ax2b.set_ylabel("Mean |Δu|")
    axes[2].grid(axis="y", alpha=0.25)
    axes[2].legend(["Control input variance", "Mean |Δu|"], loc="upper left", fontsize=9)

    fig.suptitle("Fig. 3-7 Ablation results of temporal sample enhancement", fontsize=14, y=1.03)
    _save(fig, out_dir / "Fig_3-7_Ablation_results_of_temporal_sample_enhancement")
        ...
        summary_df["Family"] = summary_df["family"].map(
            {
                ...
                "single_state_stack": "State stacking only",
                "single_n_step": "N-step only",
                "pair_action_hold_state_stack": "Action hold + state stacking",
                "pair_action_hold_n_step": "Action hold + N-step",
                "pair_state_stack_n_step": "State stacking + N-step",
                "full_temporal": "Full method",
            }
        )
        reward_pivot = summary_df.pivot(index="Family", columns="k", values="reward_loss_rate_vs_full")
        rmse_pivot = summary_df.pivot(index="Family", columns="k", values="best_eval_rmse")
        specs = [
            (reward_pivot, "Ablation heatmap of reward loss rate", "YlOrRd", "Fig_3-7_Ablation_heatmap_of_reward_loss_rate"),
            (rmse_pivot, "Ablation heatmap of RMSE", "YlGnBu", "Fig_3-7_Ablation_heatmap_of_RMSE"),
        ]
        for pivot, title, cmap, out_name in specs:
            fig_h, ax_h = plt.subplots(figsize=(9.2, 5.4))
...
def _copy_original_heatmaps(input_root: Path, out_dir: Path) -> None:
    src_root = input_root / "figure_3_7_temporal_ablation"
    mapping = {
        "ablation_heatmap_reward_loss.png": "Original_ablation_heatmap_reward_loss.png",
        "ablation_heatmap_rmse.png": "Original_ablation_heatmap_rmse.png",
    }
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Plot English version of chapter 3 result figures.")
    ...
    out_dir = (args.output_dir or (input_root / "final_figures_en")).resolve()
    ...
    _copy_original_heatmaps(input_root, out_dir)
    ...
    (out_dir / "figure_manifest_en.json").write_text(json.dumps(manifest, ensure_ascii=False, indent=2), encoding="utf-8")
