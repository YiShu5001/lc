...
BEST_V2_CHECKPOINT = PROJECT_ROOT / "outputs" / "control_pybullet_rl" / "x_refline_no_disturbance_mddpg_retrain" / "20260412_v1_to_v5_300eps_reexpanded" / "v_2" / "train" / "ladrc_x_pos_pid_att" / "x" / "20260412_145504" / "checkpoints" / "x_policy_best.pt"
...
def _plot_tracking(rows_by_label: dict[str, list[dict[str, float]]], output_path: Path) -> Path:
    ...
    plt.title("PyBullet X-Axis Tracking With Random Hover Disturbance")
    plt.grid(alpha=0.3)
    plt.legend()
    plt.tight_layout()
...
def _plot_velocity(rows_by_label: dict[str, list[dict[str, float]]], output_path: Path) -> Path:
    ...
    plt.title("PyBullet X-Axis Velocity With Random Hover Disturbance")
    plt.grid(alpha=0.3)
    plt.legend()
    plt.tight_layout()
    output_path.parent.mkdir(parents=True, exist_ok=True)
...
def _plot_error(rows_by_label: dict[str, list[dict[str, float]]], output_path: Path) -> Path:
    ...
    plt.title("PyBullet X-Axis Error With Random Hover Disturbance")
    plt.grid(alpha=0.3)
    plt.legend()
    plt.tight_layout()
    output_path.parent.mkdir(parents=True, exist_ok=True)
...
def main() -> None:
    ...
    parser.add_argument("--disturbance-scale", type=float, default=0.001)
    ...
    tag = args.tag
    output_root = PROJECT_ROOT / "outputs" / "control_pybullet" / "x_pid_ladrc_ddpg_random_hover_disturb_compare" / "x" / tag
    ...
    for eval_seed in seeds:
        reference_bundle = build_xyz_reference_trajectory(config.axis_config("x"), config, rng=np.random.default_rng(eval_seed))
    ...
    write_timeseries_csv(output_root / "ladrc_0p5_opt_timeseries.csv", rows_by_label["LADRC(0.5-opt)"])
    ...
    write_summary_json(
        output_root / "summary.json",
        {
            ...
            "controllers": {
                ...
                policy_label: {"checkpoint": str(checkpoint_path)},
            },
            "metrics": metric_rows,
