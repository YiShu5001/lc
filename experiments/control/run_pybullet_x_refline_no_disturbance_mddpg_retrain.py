...
def _plot_average_reward(rows: list[dict[str, float]], output_path: Path) -> Path:
    ...
    plt.title("No-Disturbance PyBullet RL-LADRC Average Reward vs Shared Value")
    plt.grid(alpha=0.3)
    plt.tight_layout()
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Run no-disturbance PyBullet RL-LADRC retraining with v=1 and v=5.")
    ...
    output_root = PROJECT_ROOT / "outputs" / "control_pybullet_rl" / "x_refline_no_disturbance_mddpg_retrain" / args.tag
    ...
    reward_curve_path = plot_reward_curve_collection(
        ...
        "No-Disturbance PyBullet RL-LADRC Reward Curves",
    )
