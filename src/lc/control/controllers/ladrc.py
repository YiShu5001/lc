...
class LADRCController:
    """Lightweight chapter-3 LADRC position controller with `b0 / wc / k` semantics."""
    ...
    def set_parameters(self, *, b0: float | None = None, omega_c: float | None = None, k: float | None = None) -> None:
        if b0 is not None:
            self.b0 = float(b0)
